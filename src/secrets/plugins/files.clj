(ns secrets.plugins.files
  "File and environment-based secrets plugin.
   Loads secrets from:
   - Plain EDN files (secrets.edn)
   - Encrypted EDN files (secrets.edn.enc)
   - Environment variables (SECRET__* prefix)
   
   Priority order (rightmost wins):
   home plain → home encrypted → local plain → local encrypted → environment"
...
(defn reload!
  "Reload all secrets from files and environment."
  []
  (swap! state assoc :merged (load-all-sources))
  (:merged @state))

(defn all-secrets
  "Get all loaded secrets as a merged map."
  []
  (:merged @state))

(defn get-secret
  "Get a secret by key or path.
   
   Lookup order:
   1) merged map (files + SECRET__* env)
   2) direct ENV fallback by conventional name (e.g. BRAVE_API_KEY for [:brave :api-key])"
  [k-or-path]
  (let [get-in* (fn [m k-or-path]
                  (if (vector? k-or-path)
                    (get-in m k-or-path)
                    (get m k-or-path)))]
    (or
